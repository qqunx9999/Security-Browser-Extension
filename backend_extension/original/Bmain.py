...
@author: hannousse
"""
from original import content_features as ctnfe
from original import url_features as urlfe
from original import external_features as trdfe
...
#from pandas2arff import pandas2arff comment
from urllib.parse import urlparse
from bs4 import BeautifulSoup

key = 'WAITTING FOR API & EXTENSION FINISH'
...
def getarg():
    ...
    parser.add_option("-u", "--url", dest="url", help="to specific url to extract.")
    (option,argument) = parser.parse_args()
    if (not option.url): parser.error("[-] Please specify an url. use --help for more info.")
    return option
...
def extract_features(url, status):
    ...
    if state:
        ...
        row = [url,
               ...
               #urlfe.random_domain(domain), comment
...
'''l_models = ['Random Forest', comment
                'SVM', #'XGBoost', 
                'Logistic Regression', 
                'Decision Tree', 
                'KNeighbors', 
